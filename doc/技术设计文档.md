...
5. 通过开发者模式提供脱敏、可查询且不阻塞业务的本地诊断日志。
...
- 日志查看、导出和清空必须由 Rust 校验开发者模式，不能只依赖前端隐藏。
- 日志存储独立于 SQLite；数据库或日志目录失败时应用保持可用并提供降级状态。
...
| 本地日志 | Tauri 2 日志插件或等价 Rust sink | 应用专属日志目录、大小轮转、结构化安全事件 |
...
| Diagnostics Service | 规范化、缓冲、轮转、查询和导出脱敏诊断事件 |
...
| 命令 | 输入 | 输出 |
|---|---|---|
| `get_developer_settings` | 无 | DeveloperSettingsView |
| `set_developer_mode` | enabled | DeveloperSettingsView |
| `list_diagnostics` | query | DiagnosticPage |
| `export_diagnostics` | query | DiagnosticExportResult |
| `clear_diagnostics` | 无 | DiagnosticClearResult |
...
- 开发者模式关闭时 `list_diagnostics`、`export_diagnostics` 和 `clear_diagnostics` 返回 `developer_mode_required`。
- 前端不能提交任意目标路径。
...
不在前端持久化绝对路径、API Key、开发者模式真值或 confirmation token 历史。开发者模式真值每次从 Rust 读取。

### 12.4 原型差异

- 设置页增加“常规设置 / 诊断与日志”二级视图；不新增一级路由。
- 开发者模式关闭时不渲染日志视图，Rust 仍必须拒绝相关 IPC。
...
所有可展示日志先转换为结构化 `DiagnosticRecord`：
...
`event_code`、`domain`、`recovery_code` 和其他可展示字符串必须来自枚举或 allowlist。不得提供任意 `message`、异常链或 payload 字段。`entity_ref` 由 Rust 对内部 ID 生成不可逆短引用，不使用名称或路径。

### 14.2 开发者模式

- 默认关闭，由 Rust 持久化在 Codex-O 应用专属设置中，不使用 localStorage。
- 日志始终以 Info、Warning、Error 级别安全采集，避免开启模式后无法追溯此前错误。
- 开启后允许查看、筛选、复制、导出和清空，并可提高后续运行的诊断粒度。
- 查看、导出和清空命令必须在 Rust 再次校验；前端隐藏不是权限边界。
- 开发者模式不允许关闭脱敏、读取日志目录、查看原始异常或扩大文件系统权限。

### 14.3 存储与性能

- 文件日志位于 Tauri 解析的应用专属日志目录，不写 SQLite、当前工作目录或 `~/.codex`。
- 使用大小轮转和总容量上限；建议最多 5 个文件、合计不超过 10 MiB。
- 同时保留最近 500 条的有界内存缓冲，日志目录不可用时退化为内存模式。
- 业务线程只向有界队列提交事件，文件写入由后台 worker 完成；队列饱和时优先丢弃 Debug/Info，并记录聚合丢弃计数。
- 日志服务必须早于数据库、Catalog 和设置初始化，以记录启动阶段耗时与失败。
...
用户可见错误保留自然语言说明，并附稳定错误码和事件 ID。开发者模式开启时允许跳转到对应记录。
...
事件 payload 只使用安全 ID、状态和计数；`diagnostic_event` 只推送已通过 allowlist 的 `DiagnosticRecord`。
...
10. 删除失败时恢复原文件和数据库行。
11. 开启开发者模式后查看一次脱敏失败事件，关闭后日志 IPC 被拒绝。
...
16. 诊断日志使用结构化 allowlist 和应用专属轮转文件，不写 SQLite。
17. 开发者模式只控制日志可见性和诊断粒度，Rust 负责最终授权。
