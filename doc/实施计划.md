...
- Git `main` 与 `origin/main` 位于 M1 收官提交 `3952033`。
- 现有 `observability.rs` 只有安全字段格式化模型和测试，没有日志 sink、轮转文件、内存缓冲、查询 IPC 或界面。
- 下一实施节点为 `M2-S0 诊断与日志中心`，完成后再进入 M2-S1 三段式操作框架。
...
| M2 开发 | 2026-09-07 至 2026-10-06 | 开发者模式、诊断日志、导入、quarantine、恢复、GitHub/市场、更新 |
...
- 结构化 DiagnosticRecord、稳定事件 ID、错误码和恢复建议
- 开发者模式默认关闭，由 Rust 持久化并校验日志 IPC
- 应用专属轮转日志、最近 500 条内存缓冲和日志目录失败降级
- 设置页“常规设置 / 诊断与日志”二级视图，不新增一级路由
- 启动、数据库、Catalog、扫描、AI、设置和环境健康首批埋点
- 筛选、详情、复制、脱敏导出和清空
- 异步有界写入，日志不能阻塞启动、页面返回或后台扫描

完成标准：

- 开发者模式关闭时日志查询、导出和清空 IPC 全部返回 `developer_mode_required`。
- 开启后可查看此前安全采集的事件；日志目录不可用时内存诊断仍可用。
- 日志文件、IPC、界面和导出中的密钥、正文、令牌、自由文本异常和绝对路径残留为 0。
- 现有 9 个一级路由保持不变，日志仅作为设置页二级视图。
...
- 开发者模式关闭时日志读取、导出和清空全部拒绝。
- 日志、IPC、界面和导出敏感字段残留为 0。
- 日志目录不可用时应用保持可用，内存诊断可查询。
