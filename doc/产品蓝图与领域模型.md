...
8. **可诊断但不泄密**：失败必须有稳定错误码、事件 ID 和恢复建议；日志中心始终可见，但不放宽脱敏或文件权限。
...
| 设置 | AI、扫描根目录、隐私、兼容状态；M2-S0R 增加始终可见的日志中心 | M1 |
...
- 设置页使用“概览 / 诊断事件 / 系统日志 / AI 解析日志 / Skill / MCP / 导出诊断包”二级视图；日志中心不依赖额外授权开关。
...
`SafeLogEvent` 是安全、可查询的事件协议，不负责日志落盘。

- Rust 业务和前端统一先生成结构化事件，再由 Rust 校验和规范化。
- 后端通过 `log`，前端通过统一日志封装调用 `@tauri-apps/plugin-log`；插件 formatter 对所有输入再次校验。
- 只有合法事件写入插件 JSONL；非法、超长或未知字段替换为 `frontend_log_rejected`，不保存原始内容。
- 字段固定为 `schema_version`、`event_id`、`occurred_at`、`level`、`category`、`domain`、`event_code`、`result`、`module`、`submodule`、`duration_ms`、`trace_id`、`request_ref`、`provider`、`model`、`http_status`、`item_count`、`error_code`、`retryable`、`recovery_code`、`redaction_version`。
- `trace_id` 和 `request_ref` 只保存不可逆引用；不保存原始 request ID。
- 插件写入当前日志和 4 个归档文件，单文件上限 2 MiB；文件不可用时降级 stdout，不使用内存日志兜底。
- 不保存 AI 原始响应、Prompt、请求体、API Key、Authorization、Token、绝对路径、环境变量、用户名、原始异常链、第三方响应正文或浏览器 console 文本。
- 详情页第三个页签固定为“响应元数据（脱敏）”，仅展示状态码、模型、耗时和条目数等安全字段。

### 7.9 LogViewer

`read_log_snapshot(query)` 是唯一日志读取 command。它只读取 Tauri 解析的应用日志目录，拒绝前端路径、symlink、损坏行和超长行，并返回事件、统计、筛选项、覆盖范围、cursor 与存储状态。

- 逻辑清空写入 `log_view_cleared` 标记，Viewer 忽略标记之前的事件。
- 物理清理只在用户确认并登记“下次启动时物理清理”后执行；启动时先清理本应用约定文件，再打开插件日志。
- 日志中心始终可见；历史覆盖不足以比较“较昨日”时显示“历史数据不足”，不伪造变化比例。
...
- 合法 SafeLogEvent 写入比例 100%；非法前端日志不泄漏原始内容。
- 用户可见失败关联稳定错误码和事件 ID 的比例 100%。
- 日志、日志 IPC 和诊断导出中的敏感字段残留为 0。
- 日志目录不可用时 Skill 浏览和设置基础能力可用率 100%，并明确展示“文件日志不可用”。
